    ...
    for root, subdirs, files in os.walk(args.ip_folder_name):
        ...
        for fl_n in files:
            if fl_n[-3:] == 'png':
                ...
                s_w, s_h = map(str, im.size)
                s_d = 3 if im.mode == 'YCbCr' else len(im.mode)
                s_d = str(s_d)
                
                name_id = int(fl_n.split('-')[0][3:])
                if name_id < 11:
                    name = str(name_id-1)
                elif name_id < 37:
                    name = chr(65 + name_id-11)
                else:
                    name = chr(97 + name_id-37)

                xmin, ymin, xmax, ymax = map(str, im.getbbox())
